...
export default function BottomNav() {
  ...
  return (
    <nav className="fixed bottom-4 left-4 right-4 md:right-auto md:left-4 md:top-1/2 md:-translate-y-1/2 md:bottom-auto z-50 bg-[#0A0A0A]/90 backdrop-blur-xl border border-cyan-500/20 shadow-[0_8px_32px_rgba(6,182,212,0.2)] rounded-2xl md:rounded-[40px] overflow-hidden pb-safe md:pb-0 transition-transform duration-500">
      <div className="flex flex-row md:flex-col overflow-x-auto hide-scrollbar px-1 md:px-3 py-1.5 md:py-6 items-center justify-between md:justify-start gap-1 md:gap-6">
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.name} href={item.href} className="flex-shrink-0 group">
              <div
                className={`flex flex-col items-center justify-center w-12 h-12 md:w-16 md:h-[60px] rounded-xl relative transition-all duration-300 ${
              ...
              >
                ...
                <item.icon className={`w-5 h-5 md:w-6 md:h-6 relative z-10 transition-transform duration-300 ${isActive ? 'scale-110 drop-shadow-[0_0_8px_rgba(6,182,212,0.8)]' : 'group-hover:-translate-y-1 group-hover:drop-shadow-[0_0_8px_rgba(6,182,212,0.3)]'}`} />
                <span className="text-[10px] hidden md:block relative z-10 mt-1">{item.name}</span>
              </div>
            </Link>
          )
        })}
